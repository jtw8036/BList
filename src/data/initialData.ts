import { CoupleProfile, BucketItem, MemoItem, ChallengeItem } from '../types';

export const INITIAL_PROFILE: CoupleProfile = {
  coupleCode: 'LOVE-2026',
  partner1Name: '태웅',
  partner2Name: '서주',
  anniversaryDate: '2025-05-20',
  statusMessage: '너랑 나랑 둘만의 소중한 기록 ❤️',
};

export const INITIAL_BUCKETS: BucketItem[] = [
  {
    id: "b-1",
    coupleCode: "LOVE-2026",
    title: "일본어 배워서 여행가보기",
    category: "travel",
    status: "planned",
    createdBy: "태웅",
    tags: ["여행", "배움"],
    createdAt: "2026-07-31T01:00:00.000Z",
    note: ""
  },
  {
    id: "b-2",
    coupleCode: "LOVE-2026",
    title: "클라이밍 🧗‍♀️",
    category: "sports",
    status: "planned",
    createdBy: "서주",
    tags: ["운동", "액티비티"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-3",
    coupleCode: "LOVE-2026",
    title: "용산 크리스마스공원 🎄",
    category: "date",
    status: "planned",
    createdBy: "태웅",
    tags: ["데이트", "크리스마스"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-4",
    coupleCode: "LOVE-2026",
    title: "아모레퍼시픽 [신용산] 🎪",
    category: "date",
    status: "planned",
    createdBy: "서주",
    tags: ["전시", "신용산"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-5",
    coupleCode: "LOVE-2026",
    title: "경복궁 런 🏃‍♀️ &야장 🍺",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["러닝", "야장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-6",
    coupleCode: "LOVE-2026",
    title: "올공 와인 🥂",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["올림픽공원", "와인"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-7",
    coupleCode: "LOVE-2026",
    title: "락페스티벌 룩 - 크록스 지비츠 🤘🎸",
    category: "culture",
    status: "completed",
    createdBy: "태웅",
    tags: ["락페스티벌", "패션"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-8",
    coupleCode: "LOVE-2026",
    title: "노을 버스투어 🚌",
    category: "date",
    status: "planned",
    createdBy: "서주",
    tags: ["노을", "투어"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-9",
    coupleCode: "LOVE-2026",
    title: "고카트 레이싱 🏎️",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["레이싱", "액티비티"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-10",
    coupleCode: "LOVE-2026",
    title: "일본가서 하트랜드 마시기 ❤️",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["일본", "맥주"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-11",
    coupleCode: "LOVE-2026",
    title: "도쿄오픈 도오전 🇯🇵",
    category: "travel",
    status: "planned",
    createdBy: "태웅",
    tags: ["도쿄", "도전"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-12",
    coupleCode: "LOVE-2026",
    title: "반차 쓰고 평냉 💨",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["평양냉면", "반차"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-13",
    coupleCode: "LOVE-2026",
    title: "테니스 근교 도장깨기 🎾",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["테니스", "도장깨기"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-14",
    coupleCode: "LOVE-2026",
    title: "디즈니랜드 가기 🎠",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["디즈니랜드", "여행"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-15",
    coupleCode: "LOVE-2026",
    title: "성수 태웅쓰 피자픽 🍕",
    category: "food",
    status: "completed",
    createdBy: "태웅",
    tags: ["성수", "피자"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-16",
    coupleCode: "LOVE-2026",
    title: "평냉 낮술 낮잠 zZZ",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["평냉", "낮술"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-17",
    coupleCode: "LOVE-2026",
    title: "런 🏃‍♀️ 뛰고 한강 수영장 🏊‍♂️",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["러닝", "한강수영장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-18",
    coupleCode: "LOVE-2026",
    title: "방이동 재래시장 겨울간식 🥶",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["방이동", "겨울간식"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-19",
    coupleCode: "LOVE-2026",
    title: "모어댄레귤러 캐롤 들으러 가기 🎅",
    category: "culture",
    status: "planned",
    createdBy: "태웅",
    tags: ["캐롤", "모어댄레귤러"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-20",
    coupleCode: "LOVE-2026",
    title: "등산가기 🥾 한라산 해돋이! 1/1",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["한라산", "해돋이"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-21",
    coupleCode: "LOVE-2026",
    title: "봄 인왕산 등산 흑백요리사 시즌 2 ⛰️",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["인왕산", "등산"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-22",
    coupleCode: "LOVE-2026",
    title: "가을 지리산 산장코스 ⛰️",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["지리산", "산장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-23",
    coupleCode: "LOVE-2026",
    title: "기념일 하이디라오 수타면 🍜",
    category: "food",
    status: "planned",
    createdBy: "태웅",
    tags: ["하이디라오", "기념일"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-24",
    coupleCode: "LOVE-2026",
    title: "하정우 와인 🍷",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["와인", "하정우"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-25",
    coupleCode: "LOVE-2026",
    title: "제주도 방어 여행 🏝️ ( ❗️ 소주잔 지참 ❗️ )",
    category: "travel",
    status: "planned",
    createdBy: "태웅",
    tags: ["제주도", "방어회"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-26",
    coupleCode: "LOVE-2026",
    title: "플스 F1체험 🏎️",
    category: "culture",
    status: "planned",
    createdBy: "서주",
    tags: ["플스", "F1"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-27",
    coupleCode: "LOVE-2026",
    title: "어둠속의 대화 🌚",
    category: "culture",
    status: "planned",
    createdBy: "태웅",
    tags: ["전시", "어둠속의대화"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-28",
    coupleCode: "LOVE-2026",
    title: "고베규 먹기 🍖🥢",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["고베규", "맛집"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-29",
    coupleCode: "LOVE-2026",
    title: "대전 당일 빵지순례 🍞",
    category: "travel",
    status: "planned",
    createdBy: "태웅",
    tags: ["대전", "성심당", "빵지순례"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-30",
    coupleCode: "LOVE-2026",
    title: "서울 크로와상 맛집 도장깨기 🥐",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["서울", "크로와상"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-31",
    coupleCode: "LOVE-2026",
    title: "펜싱 가보기 🤺",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["펜싱", "체험"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-32",
    coupleCode: "LOVE-2026",
    title: "지란지교+기름기뺀 고기 페어링 😆",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["페어링", "맛집"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-33",
    coupleCode: "LOVE-2026",
    title: "나이트사파리 +웅 라이온킹 브금 🐅",
    category: "travel",
    status: "planned",
    createdBy: "태웅",
    tags: ["나이트사파리", "라이온킹"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-34",
    coupleCode: "LOVE-2026",
    title: "돈키호테 ❤️선물하고 입기",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["돈키호테", "선물"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-35",
    coupleCode: "LOVE-2026",
    title: "뮤직 페스티벌 가서 와인 마시기 🍷🎉",
    category: "culture",
    status: "planned",
    createdBy: "태웅",
    tags: ["뮤직페스티벌", "와인"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-36",
    coupleCode: "LOVE-2026",
    title: "스페인 리캡 (갔던곳 핀해서 그려보기)",
    category: "travel",
    status: "completed",
    createdBy: "서주",
    tags: ["스페인", "리캡"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-37",
    coupleCode: "LOVE-2026",
    title: "스페인 리캡 샹그리아 만들기 🍊🍷",
    category: "food",
    status: "completed",
    createdBy: "태웅",
    tags: ["샹그리아", "요리"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-38",
    coupleCode: "LOVE-2026",
    title: "오키나와 🐳🐬",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["오키나와", "휴양"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-39",
    coupleCode: "LOVE-2026",
    title: "데이트 콘텐츠 인스타 계정 키우기",
    category: "goal",
    status: "planned",
    createdBy: "태웅",
    tags: ["인스타그램", "데이트콘텐츠"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-40",
    coupleCode: "LOVE-2026",
    title: "오징어 -> 수상한 꼬치 -> 석촌호수 코스 😆",
    category: "date",
    status: "planned",
    createdBy: "서주",
    tags: ["석촌호수", "데이트코스"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-41",
    coupleCode: "LOVE-2026",
    title: "서울숲 힐링 (맥주 와인 보드게임 잠) 🍺",
    category: "date",
    status: "planned",
    createdBy: "태웅",
    tags: ["서울숲", "힐링"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-42",
    coupleCode: "LOVE-2026",
    title: "테니스 전지훈련 캠핑 🎾⛺️",
    category: "sports",
    status: "planned",
    createdBy: "서주",
    tags: ["테니스", "캠핑"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-43",
    coupleCode: "LOVE-2026",
    title: "돼지불고기, 전복버터볶음밥 만들기 📖",
    category: "food",
    status: "planned",
    createdBy: "태웅",
    tags: ["요리", "홈쿠킹"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-44",
    coupleCode: "LOVE-2026",
    title: "야간매점 🌌술과 어울리는 안주만들어 파티",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["야간매점", "안주파티"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-45",
    coupleCode: "LOVE-2026",
    title: "콜플 해외콘서트 가기 👽",
    category: "culture",
    status: "planned",
    createdBy: "태웅",
    tags: ["콜드플레이", "콘서트"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-46",
    coupleCode: "LOVE-2026",
    title: "이집트 피라미드 보기 🇪🇬",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["이집트", "피라미드"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-47",
    coupleCode: "LOVE-2026",
    title: "미국 서부 트립 🇺🇸🚌",
    category: "travel",
    status: "planned",
    createdBy: "태웅",
    tags: ["미국서부", "로드트립"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-48",
    coupleCode: "LOVE-2026",
    title: "소바칸 다른 계절에 한번 더 오기 🏠",
    category: "food",
    status: "planned",
    createdBy: "서주",
    tags: ["소바칸", "맛집"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-49",
    coupleCode: "LOVE-2026",
    title: "윔블던 스트로베리크림 먹으면서 관람 🍓🎾",
    category: "sports",
    status: "planned",
    createdBy: "태웅",
    tags: ["윔블던", "테니스"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-50",
    coupleCode: "LOVE-2026",
    title: "겨울 삿포로 ❄️",
    category: "travel",
    status: "planned",
    createdBy: "서주",
    tags: ["삿포로", "겨울여행"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-51",
    coupleCode: "LOVE-2026",
    title: "여행통장 만들기 💵",
    category: "goal",
    status: "completed",
    createdBy: "태웅",
    tags: ["통장", "재테크"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-52",
    coupleCode: "LOVE-2026",
    title: "출사 📸",
    category: "culture",
    status: "completed",
    createdBy: "서주",
    tags: ["사진", "출사"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-53",
    coupleCode: "LOVE-2026",
    title: "캠핑 소고기버거&버번하이볼",
    category: "food",
    status: "completed",
    createdBy: "태웅",
    tags: ["캠핑", "버거", "하이볼"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-54",
    coupleCode: "LOVE-2026",
    title: "제미나이 초상화 그려주기 🎨",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["초상화", "그림"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-55",
    coupleCode: "LOVE-2026",
    title: "눈썰매 ☃️🛷",
    category: "sports",
    status: "completed",
    createdBy: "태웅",
    tags: ["눈썰매", "겨울"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-56",
    coupleCode: "LOVE-2026",
    title: "기념일 만들어서 애일가기 🍷",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["기념일", "와인바"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-57",
    coupleCode: "LOVE-2026",
    title: "신동엽 위스키 🥃",
    category: "food",
    status: "completed",
    createdBy: "태웅",
    tags: ["위스키", "신동엽"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-58",
    coupleCode: "LOVE-2026",
    title: "방어 안키모 소금 김밥 러시아안잭",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["방어", "안키모", "김밥"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-59",
    coupleCode: "LOVE-2026",
    title: "P의 여행 떠나기",
    category: "travel",
    status: "completed",
    createdBy: "태웅",
    tags: ["즉흥여행", "P여행"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-60",
    coupleCode: "LOVE-2026",
    title: "마라톤 🏃‍♀️🏃‍♂️ MBN 마라톤",
    category: "sports",
    status: "completed",
    createdBy: "서주",
    tags: ["마라톤", "러닝"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-61",
    coupleCode: "LOVE-2026",
    title: "초코소라빵 🍫먼저 찾는 사람이 밀로밀 사기",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["초코소라빵", "내기"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-62",
    coupleCode: "LOVE-2026",
    title: "위스키 아이스크림 먹기 🍨",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["위스키아이스크림", "디저트"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-63",
    coupleCode: "LOVE-2026",
    title: "학위복 커플사진 ❤️",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["학위복", "커플사진"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-64",
    coupleCode: "LOVE-2026",
    title: "동물원 구경 🦒",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["동물원", "나들이"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-65",
    coupleCode: "LOVE-2026",
    title: "경마공원 🐎",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["경마공원", "데이트"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-66",
    coupleCode: "LOVE-2026",
    title: "스페인포르투 서주 졸업여행(12or1) 🇪🇸🇵🇹",
    category: "travel",
    status: "completed",
    createdBy: "서주",
    tags: ["스페인", "포르투", "졸업여행"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-67",
    coupleCode: "LOVE-2026",
    title: "우드 공방 🪵 (서주 빵시리즈 2탄)",
    category: "culture",
    status: "completed",
    createdBy: "태웅",
    tags: ["우드공방", "원데이클래스"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-68",
    coupleCode: "LOVE-2026",
    title: "후토마끼랑 맥주 마시면서 피크닉 ✨",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["후토마끼", "피크닉"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-69",
    coupleCode: "LOVE-2026",
    title: "고창 꽃구경 🌼 관식 애순이 룩",
    category: "travel",
    status: "completed",
    createdBy: "태웅",
    tags: ["고창", "꽃구경"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-70",
    coupleCode: "LOVE-2026",
    title: "신당 겨울 시장 ❄️",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["신당동", "겨울시장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-71",
    coupleCode: "LOVE-2026",
    title: "필카컬러헌팅 📸",
    category: "culture",
    status: "completed",
    createdBy: "태웅",
    tags: ["필카", "사진"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-72",
    coupleCode: "LOVE-2026",
    title: "여행 기록용 음악 플리 만들기",
    category: "culture",
    status: "completed",
    createdBy: "서주",
    tags: ["플레이리스트", "음악"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-73",
    coupleCode: "LOVE-2026",
    title: "디톡스 위크! 🧄",
    category: "goal",
    status: "completed",
    createdBy: "태웅",
    tags: ["디톡스", "건강"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-74",
    coupleCode: "LOVE-2026",
    title: "추성훈 마초스테이크 먹기 🤤",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["스테이크", "추성훈"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-75",
    coupleCode: "LOVE-2026",
    title: "12월 회고록+ 랜덤 날짜정해서 일기 훔쳐보기 👀",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["회고록", "일기"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-76",
    coupleCode: "LOVE-2026",
    title: "남대문 구우경 👀",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["남대문", "구경"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-77",
    coupleCode: "LOVE-2026",
    title: "즉흥 일본 먹방 여행 🧳",
    category: "travel",
    status: "completed",
    createdBy: "태웅",
    tags: ["일본", "먹방여행"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-78",
    coupleCode: "LOVE-2026",
    title: "한강 수영장 🏊‍♂️",
    category: "sports",
    status: "completed",
    createdBy: "서주",
    tags: ["한강", "수영장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-79",
    coupleCode: "LOVE-2026",
    title: "서울 코미디 클럽 🤣",
    category: "culture",
    status: "completed",
    createdBy: "태웅",
    tags: ["코미디클럽", "공연"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-80",
    coupleCode: "LOVE-2026",
    title: "200일 기념 크록스 🙌",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["200일", "크록스"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-81",
    coupleCode: "LOVE-2026",
    title: "아쿠아리움 구경 🐧",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["아쿠아리움", "펭귄"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-82",
    coupleCode: "LOVE-2026",
    title: "청계천 야외 도서관 📖",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["청계천", "야외도서관"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-83",
    coupleCode: "LOVE-2026",
    title: "계곡 레츠꼬 📹 화채 🍉 촌캉스 🌾",
    category: "travel",
    status: "completed",
    createdBy: "태웅",
    tags: ["계곡", "촌캉스", "화채"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-84",
    coupleCode: "LOVE-2026",
    title: "에스프레소바 ☕️",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["에스프레소바", "커피"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-85",
    coupleCode: "LOVE-2026",
    title: "글램핑 🏕️ (게임 만들기ㅎ)",
    category: "travel",
    status: "completed",
    createdBy: "태웅",
    tags: ["글램핑", "게임"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-86",
    coupleCode: "LOVE-2026",
    title: "테니스 🎾 +치맥 🍗🍺",
    category: "sports",
    status: "completed",
    createdBy: "서주",
    tags: ["테니스", "치맥"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-87",
    coupleCode: "LOVE-2026",
    title: "나이트 벚꽃 빵빵런 🌸",
    category: "sports",
    status: "completed",
    createdBy: "태웅",
    tags: ["벚꽃런", "야간러닝"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-88",
    coupleCode: "LOVE-2026",
    title: "2차 맵찔이 대결 🔥🐔",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["맵찔이", "대결"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-89",
    coupleCode: "LOVE-2026",
    title: "공통 노션 만들기! 경제 공부 등 용도 🧐",
    category: "goal",
    status: "completed",
    createdBy: "태웅",
    tags: ["노션", "경제공부"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-90",
    coupleCode: "LOVE-2026",
    title: "창경궁 야간개장 ✨",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["창경궁", "야간개장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-91",
    coupleCode: "LOVE-2026",
    title: "쩜핑배틀 🤸‍♂️🤸‍♀️",
    category: "sports",
    status: "completed",
    createdBy: "태웅",
    tags: ["쩜핑배틀", "액티비티"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-92",
    coupleCode: "LOVE-2026",
    title: "도자기 공방 🏺",
    category: "culture",
    status: "completed",
    createdBy: "서주",
    tags: ["도자기", "공방"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-93",
    coupleCode: "LOVE-2026",
    title: "더현대 두바이찰떡, 쇼핑 🛍️",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["더현대", "쇼핑"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-94",
    coupleCode: "LOVE-2026",
    title: "호암미술관 전시회 니콜라스 파티 🖼️",
    category: "culture",
    status: "completed",
    createdBy: "서주",
    tags: ["호암미술관", "전시회"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-95",
    coupleCode: "LOVE-2026",
    title: "책 공유(?) 📚",
    category: "goal",
    status: "completed",
    createdBy: "태웅",
    tags: ["책공유", "독서"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-96",
    coupleCode: "LOVE-2026",
    title: "타이슨 코스 가기",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["타이슨코스", "맛집"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-97",
    coupleCode: "LOVE-2026",
    title: "사당 볼머신 쳐보기 🎾",
    category: "sports",
    status: "completed",
    createdBy: "태웅",
    tags: ["사당", "볼머신", "테니스"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-98",
    coupleCode: "LOVE-2026",
    title: "나고야 겨울 여행가기 😆😆😆(매우 기대중)",
    category: "travel",
    status: "completed",
    createdBy: "서주",
    tags: ["나고야", "겨울여행"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-99",
    coupleCode: "LOVE-2026",
    title: "겨울 반포뎅 🍡",
    category: "food",
    status: "completed",
    createdBy: "태웅",
    tags: ["반포", "오뎅"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-100",
    coupleCode: "LOVE-2026",
    title: "만화카페 주술회전 정주행 🎥",
    category: "culture",
    status: "completed",
    createdBy: "서주",
    tags: ["만화카페", "주술회전"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-101",
    coupleCode: "LOVE-2026",
    title: "클쓰마쓰 런 🎅🎄 +샌드치!?",
    category: "sports",
    status: "completed",
    createdBy: "태웅",
    tags: ["크리스마스런", "샌드위치"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-102",
    coupleCode: "LOVE-2026",
    title: "방어와 화이트와인 먹기 🐟🍷",
    category: "food",
    status: "completed",
    createdBy: "서주",
    tags: ["방어회", "화이트와인"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-103",
    coupleCode: "LOVE-2026",
    title: "잠실 크리스마스마켓 🎄 +아웃백 🥩",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["잠실", "크리스마스마켓", "아웃백"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-104",
    coupleCode: "LOVE-2026",
    title: "테니스 레슨 🎾",
    category: "sports",
    status: "completed",
    createdBy: "서주",
    tags: ["테니스", "레슨"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-105",
    coupleCode: "LOVE-2026",
    title: "찜질방 ♨️",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["찜질방", "데이트"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-106",
    coupleCode: "LOVE-2026",
    title: "호러메이즈 👻🥲",
    category: "sports",
    status: "completed",
    createdBy: "서주",
    tags: ["호러메이즈", "에버랜드"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-107",
    coupleCode: "LOVE-2026",
    title: "이우환 로스코 전시회[한강진] 🖼️🥲",
    category: "culture",
    status: "completed",
    createdBy: "태웅",
    tags: ["전시회", "한강진"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-108",
    coupleCode: "LOVE-2026",
    title: "단풍구경 🍂🍁 보러가기 🍂",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["단풍구경", "가을"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-109",
    coupleCode: "LOVE-2026",
    title: "러닝 🏃‍♀️🏃‍♂️",
    category: "sports",
    status: "completed",
    createdBy: "태웅",
    tags: ["러닝", "운동"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-110",
    coupleCode: "LOVE-2026",
    title: "올림픽 공원 꽃구경 🌼",
    category: "date",
    status: "completed",
    createdBy: "서주",
    tags: ["올림픽공원", "꽃구경"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-111",
    coupleCode: "LOVE-2026",
    title: "따릉이 한강+도시락 싸기 🚴‍♂️🚴‍♀️",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["따릉이", "도시락", "한강"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-112",
    coupleCode: "LOVE-2026",
    title: "재즈바 뚜둔 🎷",
    category: "culture",
    status: "completed",
    createdBy: "서주",
    tags: ["재즈바", "음악"],
    createdAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "b-113",
    coupleCode: "LOVE-2026",
    title: "경복궁 야간개장 🏯",
    category: "date",
    status: "completed",
    createdBy: "태웅",
    tags: ["경복궁", "야간개장"],
    createdAt: "2026-07-31T01:00:00.000Z"
  }
];

export const INITIAL_MEMOS: MemoItem[] = [
  {
    id: "m-1",
    coupleCode: "LOVE-2026",
    title: "🎄 25년 크리스마스 물음표 박스 ❓",
    content: "✅ 캘린더 만들기\n✅ 12월 주차별 다꾸",
    category: "event",
    colorTag: "rose",
    isPinned: false,
    createdBy: "서주",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T08:14:11.411Z"
  },
  {
    id: "m-2",
    coupleCode: "LOVE-2026",
    title: "🌾 촌캉스 준비물",
    content: "✅ 일기장\n✅ 수박\n✅ 시장에서 잠옷사기",
    category: "travel",
    colorTag: "amber",
    isPinned: false,
    createdBy: "태웅",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T08:14:12.129Z"
  },
  {
    id: "m-3",
    coupleCode: "LOVE-2026",
    title: "🗓️ 6월 버킷",
    content: "- 서주: 책 내용 중 말하고 싶은 부분 정리해서 말하기\n- 태웅: 퍼스널 브랜딩 관련? 뭐라도 시도해보기",
    category: "idea",
    colorTag: "emerald",
    isPinned: false,
    createdBy: "서주",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "m-4",
    coupleCode: "LOVE-2026",
    title: "📚 Book study",
    content: "- 브랜드 관해서 각자 분야에 맞는 책 읽고 얘기\n- 독립서점에서 끌리고 읽기\n- 서로 다른 매거진 b 읽고 토킹\n- 새로운 직업 책\n- 고전 벽돌책 도장깨기 (코스모스, 총균쇠)",
    category: "idea",
    colorTag: "sky",
    isPinned: true,
    createdBy: "태웅",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "m-5",
    coupleCode: "LOVE-2026",
    title: "📜 유언장 아이디어",
    content: "- 10년 주기로 일대기 잡지 편집 (장태웅 팝업 스토어/ 유골굿즈)\n- 내 영정 사진 직접 그리기",
    category: "idea",
    colorTag: "purple",
    isPinned: false,
    createdBy: "태웅",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "m-6",
    coupleCode: "LOVE-2026",
    title: "🇯🇵 나고야 버킷리스트",
    content: "✅ 히츠마부시\n✅ 야바톤 (미소철판돈까스)\n✅ 오뎅\n✅ 커피 맛집\n✅ 심야 라멘 🍜\n✅ 테바사키\n✅ 새해 다이어리 사기",
    category: "travel",
    colorTag: "rose",
    isPinned: false,
    createdBy: "서주",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T01:00:00.000Z"
  },
  {
    id: "m-7",
    coupleCode: "LOVE-2026",
    title: "🇪🇸 스페인 버킷 리스트",
    content: "✅ 심야식당 펍이나 바 가보기\n✅ 이른 아침 동네 산책\n✅ 저녁에 마트 장봐서 먹기\n✅ 시장구경+납작복숭아 🍑&디저트 와인\n✅ 맥주\n✅ 포트와인+와이너리\n✅ 한국 포트 스페인 포트 와인 비교!! - 루비 20년이상부터가 찐 👍\n✅ 이베리코, 깔솟",
    category: "travel",
    colorTag: "amber",
    isPinned: false,
    createdBy: "태웅",
    createdAt: "2026-07-31T01:00:00.000Z",
    updatedAt: "2026-07-31T01:00:00.000Z"
  }
];

export const INITIAL_CHALLENGES: ChallengeItem[] = [
  {
    id: "c-1",
    coupleCode: "LOVE-2026",
    title: "음주 챌린지 🍺",
    description: "1달에 삘데이 1번 + 간술데이 2번 (둘이 소주 1병 or 인당 맥주 1잔)",
    upgradeRule: "업그레이드: 러닝 최소 3km이상 (둘이 소주 반병 or 맥주 1잔 업그레이드 가능)",
    periodType: "monthly",
    category: "drink",
    status: "active",
    subGoals: [
      {
        id: "sg-1",
        title: "삘데이 (소주 2병 이상 등)",
        targetCount: 1,
        currentCount: 0,
        unit: "회"
      },
      {
        id: "sg-2",
        title: "간술데이 (둘이 소주 1병 or 인당 맥주 1잔)",
        targetCount: 2,
        currentCount: 0,
        unit: "회"
      }
    ],
    bonusLogs: [
      {
        id: "bl-1",
        date: "2026-07-29",
        note: "석촌호수 러닝 3.2km 달리기 완료 🏃‍♂️ -> 소주 반병 업그레이드 가능!",
        createdBy: "태웅"
      }
    ],
    createdBy: "서주",
    createdAt: "2026-07-29T11:02:00.000Z"
  }
];

export function getDefaultCoupleData(code: string) {
  const normalizedCode = code || 'LOVE-2026';
  return {
    profile: {
      ...INITIAL_PROFILE,
      coupleCode: normalizedCode,
    },
    buckets: INITIAL_BUCKETS.map((b) => ({ ...b, coupleCode: normalizedCode })),
    memos: INITIAL_MEMOS.map((m) => ({ ...m, coupleCode: normalizedCode })),
    challenges: INITIAL_CHALLENGES.map((c) => ({ ...c, coupleCode: normalizedCode })),
    trash: { buckets: [], memos: [], challenges: [] },
  };
}
